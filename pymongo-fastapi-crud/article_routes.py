...
@router.post("/", response_description="Create a new article", status_code=status.HTTP_201_CREATED, response_model=Article)
def create_article(request: Request, article: Article = Body(...)):
    ...
    created_book = request.app.database["articles"].find_one(
...
@router.get("/{id}", response_description="Get a single article by id", response_model=Article)
...
@router.put("/{id}", response_description="Update a article", response_model=Article)
